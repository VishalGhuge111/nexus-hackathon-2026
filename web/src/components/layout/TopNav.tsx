...
import { CaseDetailOverlay } from '../mission-control/CaseDetailOverlay';
...
export function TopNav() {
  ...
  useEffect(() => {
    const unsub = bus.subscribe<{ caseId: string }>('OPEN_CASE_MODAL', (payload) => {
      if (payload?.caseId) {
  ...
  return (
    <>
      <header className="h-14 bg-white border-b border-zinc-200/80 flex items-center justify-between px-6 shrink-0 z-40 relative select-none">
        ...
        {notifOpen && (
          <div className="absolute top-14 right-6 w-88 bg-white border border-zinc-200 rounded-xl shadow-xl overflow-hidden z-50 animate-fade-in">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {n.action && (
                          <button
                            onClick={() => {
                              const caseId = (n.action?.payload as Record<string, unknown>)?.caseId as string | undefined;
                              if (caseId) {
                                setSelectedCaseId(caseId);
                              } else if (n.action?.event) {
                                bus.publish(n.action.event, n.action.payload);
                              }
                              setNotifOpen(false);
                            }}
                            className="mt-2 cursor-pointer inline-flex items-center px-3 py-1 bg-zinc-900 text-white rounded-md text-xs font-semibold hover:bg-zinc-800 transition-colors shadow-2xs"
                          >
                            {n.action.label}
                          </button>
                        )}
      ...
      {/* Global Case Modal triggered by any Notification or Event */}
      {selectedCaseId && (
        <CaseDetailOverlay
