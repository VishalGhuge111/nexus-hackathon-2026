...
import { ShieldAlert, MapPin, Package, Clock, ArrowRight, PlayCircle, Loader2, AlertTriangle } from 'lucide-react';
...
export function IncidentGrid({ cases, loading = false, selectedId, onSelect }: IncidentGridProps) {
  const { openMission, activeCaseId } = useMission();
  ...
  const handleOpenMission = (id: string) => {
    setOpening(id);
    toast.info('Initializing Mission Control...', { description: 'Syncing telemetry data.' });
    setTimeout(() => {
      openMission(id);
      setOpening(null);
    }, 2000);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {cases.map((c) => (
        <div 
          ...
          className={`bg-white rounded-xl border p-5 shadow-sm hover:shadow-md transition-all flex flex-col relative overflow-hidden group ${activeCaseId === c.id || selectedId === c.id ? 'border-blue-500 ring-1 ring-blue-500' : 'border-zinc-200'}`}
        >
          {(activeCaseId === c.id || selectedId === c.id) && (
          ...
          <div className="space-y-3 mb-6">
            <div className="flex items-center gap-3 text-sm text-zinc-600">
              ...
              <span className="truncate">{c.supplierName || 'Supplier A'}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-bold text-red-600">{c.coverageDays || 2.1} Days</span>
            </div>
          </div>
          ...
          <div className="mt-auto pt-4 border-t border-zinc-100">
            <button 
              onClick={(e) => {
                e.stopPropagation();
                handleOpenMission(c.id);
              }}
              disabled={opening === c.id}
              className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${activeCaseId === c.id ? 'bg-blue-50 text-blue-700' : 'bg-zinc-900 text-white hover:bg-blue-600'}`}
            >
              {opening === c.id ? (
