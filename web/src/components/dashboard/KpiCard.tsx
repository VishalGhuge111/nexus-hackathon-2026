...
interface KpiCardProps { label: string; value: number; unit?: string; prefix?: string; icon: React.ReactNode; trend?: 'up' | 'down' | 'neutral'; status?: 'ok' | 'warn' | 'critical'; }

export function KpiCard({ label, value, unit, prefix, icon, trend, status = 'ok' }: KpiCardProps) {
  const count = useCountUp(value);
  const borderColor = status === 'critical' ? 'border-red-200' : status === 'warn' ? 'border-amber-200' : 'border-zinc-200';
  const valueColor = status === 'critical' ? 'text-red-600' : status === 'warn' ? 'text-amber-600' : 'text-zinc-900';
  ...
  return (
    <div className={`bg-white border ${borderColor} rounded-lg p-5 shadow-sm flex flex-col gap-3`}>
      ...
      <div className="flex items-end gap-1.5">
        <span className={`text-3xl font-black tabular-nums ${valueColor}`}>
