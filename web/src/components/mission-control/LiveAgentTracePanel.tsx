...
export function LiveAgentTracePanel({
  ...
  return (
    <Panel title="Live Agent Trace" subtitle="Figure 2 — Agent Execution FSM (§10)" tone="primary">
      ...
      <div className="mt-4 flex items-center gap-6 border-t border-zinc-100 pt-3 text-xs text-zinc-400">
        <span>
          cycle <span className="font-mono text-zinc-700">{agentState.cycle}</span>
        </span>
        <span>
          tool-call budget{" "}
        ...
        {agentState.lastLlmModel && (
          <span>
            last LLM <span className="font-mono text-zinc-700">{agentState.lastLlmModel}</span>
          </span>
        )}
      </div>
